...
`RELEASES-RATIONALE.md` § Why backport main → dev after publish for why). Without this step `dev` keeps the pre-release
`VERSION` indefinitely and feature branches cut from `dev` inherit a stale baseline.
...
→ Pre-release gating: [`RELEASES-PREFLIGHT.md`](./RELEASES-PREFLIGHT.md) (run before step 1 of `## Releasing dev to
main`).
